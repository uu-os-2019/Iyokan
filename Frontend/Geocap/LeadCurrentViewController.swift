...
class LeadCurrentViewController: UIViewController, UITableViewDataSource {
    
    @IBOutlet weak var tableLead: UITableView!
    
    var leaderboard = geoCap.server.getLeaderboardCurrent()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.points.text = "XP/S: " + "\(leaderboard?.users[indexPath.row].exp_rate ?? 0)"
